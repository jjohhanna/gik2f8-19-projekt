...
Adressen som skickas in är http://localhost:5000/allRecept och innan det fungerar är det viktigt att ändra det i servern. I Lektion 5 sattes alla routes till /newRecept. Dessa ska ändras till /allRecept. Dessa routes är första argumenten till app.get, app.post och app.delete, så det ser ut ungefär app.get("/newRecept",...). Alla sådana ska ändras till "/allRecept". */
...
function saveNewRecept() {
  /* Ett objekt vid namn newRecept byggs ihop med hjälp av formulärets innehåll */
  ...
  Callbackfunktionen som används i then() är en anonym arrow-function som tar emot innehållet i det promise som create returnerar och lagrar det i variabeln newRecept. 
  */

  api.create(newRecept).then((newRecept) => {
    /* newRecept kommer här vara innehållet i promiset. Om vi ska följa objektet hela vägen kommer vi behöva gå hela vägen till servern. Det är nämligen det som skickas med res.send i server/api.js, som api-klassens create-metod tar emot med then, översätter till JSON, översätter igen till ett JavaScript-objekt, och till sist returnerar som ett promise. Nu har äntligen det promiset fångats upp och dess innehåll - uppgiften från backend - finns tillgängligt och har fått namnet "newRecept".  */
    if (newRecept) {
      /* När en kontroll har gjorts om newRecept ens finns - dvs. att det som kom tillbaka från servern faktiskt var ett objekt kan vi anropa renderList(), som ansvarar för att uppdatera vår todo-lista. renderList kommer alltså att köras först när vi vet att det gått bra att spara ner den nya uppgiften.  */
      renderList();
    }
  });
}
  ...
  api.getAll().then((allRecept) => {
    ...
    if (allRecept && allRecept.length > 0) {
      ...
      allRecept.forEach((newRecept) => {
        ...
        4. rendernewRecept(newRecept) - funktion som returnerar HTML. 
        5. newRecept (objekt som representerar en uppgift som finns i arrayen) skickas in till rendernewRecept, för att rendernewRecept ska kunna skapa HTML utifrån egenskaper hos uppgifts-objektet. 
        */

        /* Denna kod körs alltså en gång per element i arrayen allRecept, dvs. en  gång för varje uppgiftsobjekt i listan. */
        todoListElement.insertAdjacentHTML('beforeend', renderNewRecept(newRecept));
      });
    }
  });
}

/* render newRecept är en funktion som returnerar HTML baserat på egenskaper i ett uppgiftsobjekt. 
...
/* Destructuring används för att endast plocka ut vissa egenskaper hos uppgifts-objektet. Det hade kunnat stå function renderNewRecept(newRecept) {...} här - för det är en hel newRecept som skickas in - men då hade man behövt skriva newRecept.id, newRecept.title osv. på alla ställen där man ville använda dem. Ett trick är alltså att "bryta ut" dessa egenskaper direkt i funktionsdeklarationen istället. Så en hel newRecept skickas in när funktionen anropas uppe i todoListElement.insertAdjacentHTML("beforeend", renderNewRecept(newRecept)), men endast vissa egenskaper ur det newRecept-objektet tas emot här i funktionsdeklarationen. */
function renderNewRecept({ id, title, allergy}) {
  /* Baserat på inskickade egenskaper hos newRecept-objektet skapas HTML-kod med styling med hjälp av tailwind-klasser. Detta görs inuti en templatestring  (inom`` för att man ska kunna använda variabler inuti. Dessa skrivs inom ${}) */
  ...
  */
  ...
  let html = `
    <li class="select-none mt-2 py-2 border-b border-pink-300 ${newReceptColor}">
      <div class="flex items-center">
      <h3 class="mb-3 flex-1 text-xl font-bold uppercase">${title}</h3>
        <div>
          <button onclick="deleteNewRecept(${id})" class="inline-block bg-violet-300 text-xs text-violet-700 border border-pink px-3 py-1 rounded-md ml-2">Ta bort</button>
        </div>
      </div>`;
  ...
  allergy &&
    ...
    /* Det som ska göras om dallergy finns är att html-variabeln ska byggas på med HTML-kod som visar det som finns i allergy-egenskapen hos newRecept-objektet. */
    (html += `
