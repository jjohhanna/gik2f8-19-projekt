...
app.get('/allRecept', async (req, res) => {
  ...
  try {
    /* Node har en inbyggd modul som heter fs (importerades i början av denna fil). Den används här för att försöka läsa innehållet i en fil vid namn allRecept.json. Anropet är asynkront så man sätter await innan (och async innan callbackfunktionen i app.get().) */
    const allRecept = await fs.readFile('./allRecept.json');
...
app.post('/allRecept', async (req, res) => {
  try {
    ...
    const oneRecept = req.body;
    /* Det befintliga innehållet i filen läses in och sparas till variabeln listBuffer. */
    const listBuffer = await fs.readFile('./allRecept.json');
    /* Innehållet i filen är de uppgifter som hittills är sparade. JSON.parse behövs för att kunna behandla listan av Recept i filen som ett JavaScript-objekt. Parse används för att översätta en buffer eller text till JavaScript */

    const currentAllRecept = JSON.parse(listBuffer);
    /* Skapar en variabel för att kunna sätta id på den nya uppgiften */
    let maxOneReceptId = 1;
    /* Om det finns några uppgifter sedan tidigare, dvs. currentAllRecept existerar och är en lista med en längd större än 0 ska ett nytt id räknas ut baserat på de som redan finns i filen */
    if (currentAllRecept && currentAllRecept.length > 0) {
      /* Det görs genom array.reduce() som går igenom alla element i listan och tar fram det högsta id:t. Det högsta id:t sparas sedan i variabeln maxOneReceptId */
      maxOneReceptId = currentAllRecept.reduce(
        /* För varje element i currentAllRecept anropas en callbackfunktion som får två parametrar, maxId och currentElement. maxId kommer att innehålla det id som för närvarande är högst och currentElement representerar det aktuella element i currentAllRecept som man för närvarande kontrollerar.  */
        (maxId, currentElement) =>
          /* Om id:t för den aktuella uppgiften är större än det i variabeln maxId, sätts maxId om till det id som nu är högst. maxId är från början satt till värdet av maxOneReceptId (1, enligt rad 53.).  */
    ...
    const newRecept = { id: maxOneReceptId + 1, ...oneRecept };
    /* Om currentAllRecept finns - dvs det finns tidigare lagrade uppgifter,  skapas en ny array innehållande tidigare uppgifter (varje befintlig uppgift i currentAllRecept läggs till i den nya arrayen med hjälp av spreadoperatorn) plus den nya uppgiften. Om det inte tidigare finns några uppgifter, skapas istället en ny array med endast den nya uppgiften.  */
    const newList = currentAllRecept ? [...currentAllRecept, newRecept] : [newRecept];

    /* Den nya listan görs om till en textsträng med hjälp av JSON.stringify och sparas ner till filen allRecept.json med hjälp av fs-modulens writeFile-metod. Anropet är asynkront så await används för att invänta svaret innan koden går vidare. */
    await fs.writeFile('./allRecept.json', JSON.stringify(newList));
...
app.delete('/allRecept/:id', async (req, res) => {
  console.log(req);
  try {
    ...
    if (currentAllRecept.length > 0) {
      /* Om det finns något i filen görs här en hel del i samma anrop: 
      1. De befintliga uppgifterna (currentAllRecept), filtreras så att den uppgift med det id som skickades in filtreras bort och endast de uppgifter som inte hade det id:t är kvar.
      2. Arrayen med alla uppgifter utom den med det id som skickades in görs om till en sträng med JSON.stringify
      3. Denna sträng sparas slutgilgingen till filen allRecept.json, så att det kommer att finnas en uppdaterad lista som inte längre innehåller uppgiften med det id som skickades in via url:en. */
      await fs.writeFile(
        './allRecept.json',
        JSON.stringify(currentAllRecept.filter((oneRecept) => oneRecept.id != id))
      );
